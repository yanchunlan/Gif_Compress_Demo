...
static jint bufferUpTo(JNIEnv *env, StreamContainer *sc, size_t size) {
    ...
    jint length;
    do {
        length = (*env)->CallIntMethod(env, sc->stream, sc->readMethodID, sc->buffer, totalLength,
                                       size - totalLength);
        if (length > 0) {
            totalLength += length;
        } else {
...
//    LOGE("4 so层 正常");
    ...
    if (info == NULL) {
//        LOGE("5 so层 createGifInfoFromFile == null");
        fclose(file);
    }
    return (jlong) (intptr_t) info;
}

jint getWidth(jlong info){
    GifInfo *gifInfo = (GifInfo *) (intptr_t) info;
    return gifInfo->originalWidth;
}

jint getHeight(jlong info){
    GifInfo *gifInfo = (GifInfo *) (intptr_t) info;
...
__unused JNIEXPORT jlong JNICALL
Java_pl_droidsonroids_gif_GifInfoHandle_openDirectByteBuffer(JNIEnv *env, jclass __unused class,
                                                             jobject buffer) {
...
Java_pl_droidsonroids_gif_GifInfoHandle_openStream(JNIEnv *env, jclass __unused class,
    ...
    if (markMethodID == NULL || container->readMethodID == NULL ||
        container->resetMethodID == NULL || container->closeMethodID == NULL) {
        (*env)->DeleteGlobalRef(env, container->buffer);
        free(container);
        return NULL_GIF_INFO;
    }
...
Java_pl_droidsonroids_gif_GifInfoHandle_extractNativeFileDescriptor(JNIEnv *env,
                                                                    jclass __unused handleClass,
                                                                    jobject fileDescriptor,
                                                                    jboolean closeOriginalDescriptor) {
...
__unused JNIEXPORT jint JNICALL
Java_pl_droidsonroids_gif_GifInfoHandle_createTempNativeFileDescriptor(JNIEnv __unused *env,
                                                                       jclass __unused handleClass) {
    return eventfd(0, 0);
}

__unused JNIEXPORT jlong JNICALL
Java_pl_droidsonroids_gif_GifInfoHandle_openNativeFileDescriptor(JNIEnv *env,
                                                                 jclass __unused handleClass,
